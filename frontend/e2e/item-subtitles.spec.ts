...
test.describe("Item Subtitles", () => {
  ...
  test.describe("Items Panel", () => {
    test("displays subtitle in items panel when category is selected", async ({
      ...
      await page.route("**/api/v1/items?*", async (route) => {
        ...
        let filteredItems = fixtures.testItems;
        if (categoryId) {
          filteredItems = fixtures.testItems.filter(
  ...
  test.describe("Edge Cases", () => {
    test("handles items with partial specifications", async ({ page }) => {
      ...
      await page.route("**/api/v1/items?*", async (route) => {
        const partialSpecItem = {
          ...
          is_low_stock: false,
          tags: [],
          category: fixtures.testCategories[1],
        ...
        await route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            items: [partialSpecItem, ...fixtures.testItems],
            total: fixtures.testItems.length + 1,
            page: 1,
            limit: 20,
            total_pages: 1,
          }),

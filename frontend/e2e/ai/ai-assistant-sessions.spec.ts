...
test.describe("AI Assistant Sessions", () => {
  ...
  test.describe("Session Creation", () => {
    ...
    test("uses example query to start session", async ({ page, network }) => {
      // Override sessions endpoint to return empty list
      network.use(
        http.get("**/api/v1/ai/sessions", () => {
          return HttpResponse.json({
  ...
  test.describe("Insufficient Credits", () => {
    test("shows insufficient credits modal on 402 error", async ({
      page,
      network,
    }) => {
      await page.goto("/ai-assistant");
      ...
      // Override chat endpoint to return 402
      network.use(
  ...
  test.describe("Error Handling", () => {
    test("shows error message on chat failure", async ({ page, network }) => {
    ...
    test("handles session loading error gracefully", async ({
      page,
      network,
    }) => {

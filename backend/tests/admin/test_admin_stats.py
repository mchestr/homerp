...
class TestAdminStats:
    ...
    def test_sorting_timezone_aware_timestamps(self):
        """Test that sorting works with timezone-aware timestamps.

        All database timestamps are now stored with timezone info (TIMESTAMP WITH TIME ZONE),
        so sorting should work directly without any normalization.
        """
        from src.admin.schemas import RecentActivityItem

        # Create activity items with timezone-aware timestamps
        now = datetime.now(UTC)
        activities = [
            RecentActivityItem(
                id=uuid.uuid4(),
            ...
            RecentActivityItem(
                ...
                type="signup",
                title="User 2",
                description="Second most recent user",
                user_email="user2@example.com",
                user_name="User 2",
                timestamp=now - timedelta(minutes=5),  # 5 min ago
            ),
            RecentActivityItem(
                id=uuid.uuid4(),
                type="feedback",
        ...
        # Sorting should work directly with timezone-aware timestamps
        activities.sort(key=lambda x: x.timestamp, reverse=True)

        # Verify sorting worked and newest items are first
        assert len(activities) == 4
        assert activities[0].title == "User 1"
        assert activities[1].title == "User 2"
        assert activities[2].title == "Feedback 1"
        assert activities[3].title == "Feedback 2"
